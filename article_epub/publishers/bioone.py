...
class BioOne(Publisher):
    ...
    def get_doi(self):
        if self.doi == None:
            doi_raw = self.soup.find("p", class_="articleRef").find("a").text.split("/")
            self.doi = str(doi_raw[3] + "/" + doi_raw[4])

    def get_abstract(self):
        """Get article abstract"""
        abstract_raw = str(self.soup.find("div", class_="abstractSection"))
        self.abstract = abstract_raw.replace("<h3", "<h2").replace("</h3>", "</h2>").replace("Abstract. ", "ABSTRACT")
    ...
    def get_body(self):
        ...
        for i in imgs_old:
            try:
                ...
                imgpage = BeautifulSoup(requests.get("https://bioone.org" + str(link["href"])).content, "html.parser")
    ...
    def get_references(self):
        ...
        self.references = references_raw.replace("<h3>", "<h2>").replace("</h3>", "</h2>")
