...
class RoyalSociety(Publisher):
    ...
    def get_doi(self):
        if self.doi == None:
            self.doi = str(self.soup.find("span", class_="highwire-cite-metadata-doi").text.split(" ")[1])
    ...
    def get_keywords(self):
        ...
        self.keywords = []
        try:
            keywords_raw = self.soup.find("div", class_="pane-node-field-highwire-article-keyword").find_all("a")
            self.keywords = []
    ...
    def get_body(self):
        ...
        for i in tables:
            try:
                src = "http://rstb.royalsocietypublishing.org" + i.find("a")["data-table-url"]
                dat = requests.get(src, headers={"User-Agent": "Mozilla/5.0"})
                tabsoup = BeautifulSoup(dat.content, "html.parser").find("table")
                i.append(tabsoup)
    ...
    def get_references(self):
        """Get references list"""
        references_raw = self.soup.find("div", {"id": "ref-list-1"}).find("ol").find_all("li", recursive=False)
