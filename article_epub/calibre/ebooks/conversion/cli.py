...
from article_epub.calibre import patheq
from article_epub.calibre.customize.conversion import OptionRecommendation
from article_epub.calibre.ebooks.conversion import ConversionUserFeedBack
from article_epub.calibre.utils.config import OptionParser
from article_epub.calibre.utils.localization import localize_user_manual_link
from article_epub.calibre.utils.logging import Log

# no time to implement gettext
_ = str
...
def add_pipeline_options(parser, plumber):
    ...
    for group, (desc, options) in iter(groups.items()):
        if group:
...
def create_option_parser(args, log):
    if '--version' in args:
        from article_epub.calibre.constants import __appname__, __author__, __version__
        log(os.path.basename(args[0]), '('+__appname__, __version__+')')
        log('Created by:', __author__)
        raise SystemExit(0)
    if '--list-recipes' in args:
        from article_epub.calibre.web.feeds.recipes.collection import get_builtin_recipe_titles
    ...
    from article_epub.calibre.ebooks.conversion.plumber import Plumber
...
def main(args=sys.argv):
    ...
    if opts.transform_css_rules:
        from article_epub.calibre.ebooks.css_transform_rules import import_rules, validate_rule
    ...
    if opts.transform_html_rules:
        from article_epub.calibre.ebooks.html_transform_rules import import_rules, validate_rule
